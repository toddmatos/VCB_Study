...
function VCB_BF_RepositioningAndResizing()
  ...
  --[[240630.19.23 quoted out and update
  ]]--
  if grayedIcons then
    for i=0,10 do
...
function VCB_BF_GetDuration(timeLeft)
  if not VCB_IS_LOADED then
    return
  end

  local hours, minutes, seconds = math.floor(timeLeft/3600), math.floor(math.fmod(timeLeft,3600)/60), math.floor(math.fmod(timeLeft, 60))

  -- Use string.format for efficient string formatting
  if hours >0 then
